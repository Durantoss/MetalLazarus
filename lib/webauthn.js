...
export async function finishRegistration(userId, responseJSON) {
    const record = await prisma.webAuthnChallenge.findFirst({
        where: { userId, type: 'registration' },
        orderBy: { createdAt: 'desc' },
    });
    if (!record)
        throw new Error('No registration challenge');
    let verification;
...
export async function startAuthentication(userId) {
...
export async function finishAuthentication(userId, responseJSON) {
    const record = await prisma.webAuthnChallenge.findFirst({
        where: { userId, type: 'authentication' },
        orderBy: { createdAt: 'desc' },
    });
    if (!record)
        throw new Error('No authentication challenge');
    const credId = responseJSON.id;
